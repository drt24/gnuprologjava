...
test_92 :- 
       ...
       ->
       (
     	error_test(X52 is +(MI,1),evaluation_error(int_overflow)),
     	error_test(X53 is -(+(MI,1),1),evaluation_error(int_overflow)),
     	error_test(X54 is -(-2,MI),evaluation_error(int_overflow)),
     	error_test(X55 is *(MI,2),evaluation_error(int_overflow)),
     	R is float(MI) * 2,
     	error_test(X56 is floor(R),evaluation_error(int_overflow))
      )
       ;
     true
     ),
